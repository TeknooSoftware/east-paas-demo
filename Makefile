...
lint:
	...
	find ./domains -name "*.php" -exec /usr/bin/env php -l {} \; | grep "Parse error" > /dev/null && exit 1 || exit 0
	find ./infrastructures -name "*.php" -exec /usr/bin/env php -l {} \; | grep "Parse error" > /dev/null && exit 1 || exit 0

phploc:
	vendor/bin/phploc src
	vendor/bin/phploc domains
	vendor/bin/phploc infrastructures

phpstan:
	vendor/bin/phpstan analyse src domains infrastructures --level max

phpcs:
	vendor/bin/phpcs --standard=PSR12 --extensions=php src/
	vendor/bin/phpcs --standard=PSR12 --extensions=php domains/
	vendor/bin/phpcs --standard=PSR12 --extensions=php infrastructures/

phpcpd:
	vendor/bin/phpcpd src/
	vendor/bin/phpcpd domains/
